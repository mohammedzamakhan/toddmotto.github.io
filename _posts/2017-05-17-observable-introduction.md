...
When we subscribe to an Observable, it will keep passing any values to an observer, until it's no longer needed. At the point where it's no longer needed, the Observable can clean up itself, calling `.complete()` when finished.
